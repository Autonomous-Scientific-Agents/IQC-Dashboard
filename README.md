...
- Works directly with Parquet datasets generated by IQC and supported JSON exports
...
Energy values are displayed in `kcal/mol` by default. Use the Energy units
control at the top of the dashboard to switch between `kcal/mol` and `eV`.

Parquet and supported JSON files can be uploaded from the sidebar. Local JSON
or Parquet paths can also be loaded when launching through `streamlit_app.py`:
```bash
...
```bash
docker pull ghcr.io/autonomous-scientific-agents/iqc_dashboard:latest
docker run --rm -p 8501:8501 ghcr.io/autonomous-scientific-agents/iqc_dashboard:latest
```

To build the same image locally:

```bash
docker build -t iqc-dashboard .
docker run --rm -p 8501:8501 iqc-dashboard
```

### Expected Data Schema

The dashboard expects Parquet files generated by [IQC](https://github.com/Autonomous-Scientific-Agents/IQC) with the following columns (at minimum). It can also ingest JSON table exports; supported reaction JSON files with `reactant_geometry` and `product_geometry` are converted into reactant/product molecule rows internally.
...
- `spectrum_frequencies`: IR spectrum frequency grid
- `spectrum_frequencies_units`: Units for `spectrum_frequencies`
- `spectrum_intensities`: IR spectrum intensities aligned with `spectrum_frequencies`
- `spectrum_intensities_units`: Units for `spectrum_intensities`
- `frequencies_cm^-1`: Raw vibrational frequencies, when available
